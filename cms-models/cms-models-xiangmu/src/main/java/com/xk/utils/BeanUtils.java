...
public class BeanUtils {
    ...
    /**
     * 判断一个对象中的所有字段是否都为 null
     * @param bean
     * @return 如果所有字段都为 null 返回 true，否则返回 false
     */
    public static boolean areAllFieldsNull(Object bean) {
        if (bean == null) {
        ...
        try {
            ...
            for (Field field : fields) {
                ...
                if (value != null) {
                    field.setAccessible(false);// 访问完后恢复原值
                    return false; // 如果字段值不为 null，直接返回 false
                }
                field.setAccessible(false);// 访问完后恢复原值
            }
    ...
    public static void main(String[] args) {
        ...
        boolean result1 = BeanUtils.areAllFieldsNull(bean1);
        System.out.println("1: " + result1); // 预期输出: true

        // 测试用例2: 有一个字段为 null
        TestBean bean2 = new TestBean(null, 123, new Object());
        boolean result2 = BeanUtils.areAllFieldsNull(bean2);
        System.out.println("2: " + result2); // 预期输出: false

        // 测试用例3: 所有字段都为 null
        TestBean bean3 = new TestBean(null, null, null);
        boolean result3 = BeanUtils.areAllFieldsNull(bean3);
        System.out.println("T3: " + result3); // 预期输出: false

        // 测试用例4: 对象本身为 null
        TestBean bean4 = null;
        boolean result4 = BeanUtils.areAllFieldsNull(bean4);
        System.out.println("4: " + result4); // 预期输出: false
    }
